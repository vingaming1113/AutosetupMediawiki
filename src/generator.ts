...
This project automatically runs Cap Standalone at ${config.captcha.serverUrl}. When the wiki starts, it creates a site key with ${new URL(config.siteUrl).origin} as its allowed origin before MediaWiki starts.
...
The installer creates \`LocalSettings.php\` once. Later starts can use \`docker compose up -d\` without running the installer again.
